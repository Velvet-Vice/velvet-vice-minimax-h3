...
def ensure_taeh3_decoder() -> tuple[bool, str]:
    ...
    with _DOWNLOAD_LOCK:
        ...
        try:
            ...
            request = Request(TAEH3_URL, headers={"User-Agent": "VelvetViceMiniMaxH3/1.4.3"})
            total = 0
            ...
            digest = _sha256(temp)
            if digest != TAEH3_SHA256:
                raise RuntimeError(f"SHA256 verification failed. Expected {TAEH3_SHA256}, got {digest}.")
            os.replace(temp, target)
            _LAST_RESULT = (True, f"TAEHV decoder installed automatically: {target}")
            logging.info("VELVET VICE H3 PREVIEW | %s", _LAST_RESULT[1])
            return _LAST_RESULT
        except Exception as error:
            _LAST_RESULT = (False, f"TAEHV automatic download failed: {type(error).__name__}: {error}")
